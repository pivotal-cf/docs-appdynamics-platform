...
1. Doubleclick the **AppDynamics Platform Monitoring for VMware Tanzu** tile in the Installation Dashboard.
...
* **UAA User**: Username of the UAA account with access to log data. The account must belong to `logs.admin` authority. Please refer these [instructions] (https://github.com/cf-platform-eng/firehose-nozzle-v2#authentication--testing) to create a UAA account.
* **UAA Secret**: The secret key of the UAA User account.
...
#### v4.x Nozzle Configuration
![v4.x Settings tab for AppDynamics Nozzle Configuration](./images/nozzle_config.png)

* **Doppler Logging Endpoint**: The Firehose endpoint address for the `doppler_logging_endpoint` value.
...
Depending on the version of the AppDynamics Platform Monitoring for VMware Tanzu tile, there are two options to authenticate a user:

*  v5.x Nozzle Credentials
*  v4.x Nozzle Credentials

#### v5.x Nozzle Credentials

Version 5.x of the AppDynamics Platform tile uses a **UAA User** and **UAA Secret** instead of a **Doppler Logging Endpoint, Username**, and **Password**.
...
To use the existing **opentsdb-firehose-nozzle** account shipped with Cloud Foundry(CF):

1. Log in to the **OPS Manager** and select the **TAS** tile.
1. Navigate to **Credentials > UAA > Opentsdb Nozzle Credentials**.
1. Copy the username and password.

(or)

* Create a new account in the **doppler.firehose group** with the correct permissions.
...
Metrics|rep|cf|diego_cell|<guid>|rep. CapacityRemainingMemory as shown below. Doubleclick it to
